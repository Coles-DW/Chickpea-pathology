...
workingDir = ""
setwd(workingDir)
data=read.table("Chickpeatimecourse_rawexpresseddata.txt", header = TRUE, row.names = 1)
colnames(data)
...
write.table(colData, file="ChickpeaCol_Samples_Conditions.txt", sep="\t")
######boxplot of data transnformation
epsilon <- 1 # pseudo-count to avoid problems with log(0)
hist(as.matrix(log2(data + epsilon)), breaks=100, col="blue", border="white",
     main="Expressed_Log2-transformed counts per gene_Chickpea", xlab="log2(counts+1)", ylab="Number of genes", 
     las=1, cex.axis=0.7)
dev.copy(png,'Log2-transformed_Histogram_chickpea.png')
dev.off()
...
dev.copy(png,'boxplot_expressed_transformeddata_Chickpea.png')
dev.off()
######dds=data deseq
...
write.table(norm_counts, file="chickpea_normalized_expressed.txt", sep="\t")
##rlog tranformation
...
dev.copy(png,'Expressed_Chickpea_sampletosample_heatmap.png')
dev.off()
print(plotPCA(rld))
...
ggplot(pcaData, aes(PC1, PC2, color=condition)) +
  ...
  theme_classic()
dev.copy(png, 'Expressed_chickpea_PCA2')
dev.off()
...
write.table(r, file="Chickpea_correlation_values.txt", sep="\t")
## calcul de la dissimilarité ##
d=1-r
## Clustering ##
h = hclust(as.dist(d), method = "complete", members = NULL)
plot(h)
dev.copy(png, 'Expressed_Chickpea_hclust')
dev.off()
...
write.table(chickpeageneexpressionresults, file="Differentialexpression_chickpea.txt", sep="\t")
