...
gene2GO_map <- readMappings(file="Gene2GO_Sep2019_v2.txt", sep = "\t")
#Defining your list of genes of interest, and the 'gene universe' 
geneUniverse<-names(gene2GO_map)
head(geneUniverse)
genesOfInterest<- read.table("All_sig_up_24.txt",header=FALSE) #Genes you are interested in 
...
write.table(allRes, file="72_GOenrich_combined_unique_sig_up_down.csv", sep=",")
